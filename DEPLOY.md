...
| **Postgres host** (you have one) | DB with `pgvector` + `uuid-ossp` extensions + FTS (built in) | Supabase/Neon free tiers work |
...
| ~~Elasticsearch~~ | Replaced by Postgres FTS — skip Elastic Cloud | — |
...
### 1.4 Elasticsearch — SKIP

We use Postgres FTS instead (tsvector + GIN index on `schemes.search_doc`,
created by migration `20260618000000_scheme_fts_tsvector`). Don't sign up
for Elastic Cloud and don't set `ELASTICSEARCH_*` env vars. The hybrid
search pipeline detects the absence and runs Postgres FTS + Pinecone vector
search instead. `/readyz` skips the Elasticsearch check when the env var
is missing.

If you ever want to re-enable Elasticsearch for richer keyword behaviour,
set `ELASTICSEARCH_NODE` (plus username/password or API key) and the
indexer + readiness probe re-enable themselves automatically.
